...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            "session_id_str", sa.String(36), nullable=False
        ),  # Denormalized for queries
        sa.Column("sequence_number", sa.BigInteger(), nullable=False),
        sa.Column(
            "timestamp_wall", postgresql.TIMESTAMP(timezone=True), nullable=False
        ),
    ...
    op.create_index(
        "idx_session_sequence_unique",
        "event_chains",
        ["session_id", "sequence_number"],
        unique=True,
    )
    op.create_index(
        "idx_event_type_timestamp", "event_chains", ["event_type", "timestamp_wall"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "seal_timestamp", postgresql.TIMESTAMP(timezone=True), nullable=False
        ),
    ...
    op.add_column(
        "sessions",
        sa.Column("session_id_str", postgresql.UUID(as_uuid=True), nullable=True),
    )
    op.add_column(
        "sessions",
        sa.Column(
            "chain_authority",
            sa.Enum("server", "sdk", name="chain_authority_enum"),
            nullable=True,
        ),
    )
    ...
    op.add_column(
        "sessions", sa.Column("ingestion_service_id", sa.String(100), nullable=True)
    )

    # Add constraints
    op.create_check_constraint(
        "valid_chain_authority", "sessions", "chain_authority IN ('server', 'sdk')"
    )
