...
def seal_chain(
    ...
    existing_seal: dict[str, Any] | None = None,
    total_drops: int = 0,
) -> SealResult:
    """
...
def reject_post_seal_event(
    session_id: str, seal_timestamp: str, event_timestamp: str
) -> SealResult:
    """
...
def _compute_session_digest(
    session_id: str, events: list[dict[str, Any]], final_hash: str
) -> str:
    """
...
def create_chain_seal_event(seal_result: SealResult, session_id: str) -> dict[str, Any]:
    ...
    if seal_result.status != SealStatus.SEALED:
        raise ValueError(
            f"Cannot create CHAIN_SEAL event from non-sealed result: {seal_result.status}"
        )
